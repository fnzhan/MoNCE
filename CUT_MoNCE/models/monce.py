...
class MoNCELoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        f = OT(ot_q, ot_k, eps=self.opt.eps, max_iter=50, cost_type=self.opt.cost_type)
        f = f.permute(0, 2, 1) * (self.opt.num_patches - 1) * self.opt.neg_term_weight + 1e-8
        ...
        # if self.loss_type == 'MoNCE':
        l_neg_curbatch = l_neg_curbatch + torch.log(f) * self.opt.nce_T
